<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}' class="title">进场停车</view>
				<picker @change="jinchangtingcheChange" :value="jinchangtingcheIndex" :range="jinchangtingcheOptions">
					<view :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 152, 242, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{ruleForm.jinchangtingche?ruleForm.jinchangtingche:"请选择进场停车"}}</view>
				</picker>
			</view>
												<view :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"#ccc","margin":"0 0 20rpx 0","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group">
				...
				<input :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 152, 242, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"110rpx"}' :disabled="ro.xiaoshidanjia" v-model="ruleForm.xiaoshidanjia" placeholder="小时单价"></input>
												...
												<view :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"#ccc","margin":"0 0 20rpx 0","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group">
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}' class="title">进场时间</view>
				<input :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 152, 242, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"110rpx"}' v-model="ruleForm.jinchangshijian" placeholder="进场时间" @tap="toggleTab('jinchangshijian')"></input>
			</view>
												<view :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"#ccc","margin":"0 0 20rpx 0","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group">
			...
			<view class="btn">
				<button :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,0) inset","backgroundColor":"#409EFF","borderColor":"#409EFF","borderRadius":"8rpx","color":"#fff","borderWidth":"2rpx","width":"160rpx","fontSize":"28rpx","borderStyle":"solid","height":"80rpx"}' @tap="onSubmitTap" class="bg-red">提交</button>
																										...
																										<w-picker mode="dateTime" step="1" :current="false" :hasSecond="false" @confirm="jinchangshijianConfirm" ref="jinchangshijian" themeColor="#333333"></w-picker>
...
<script>
	...
	export default {
				...
				ruleForm: {
												dingdanbianhao: this.getUUID(),
																...
																userid: '',
																																				...
																																				jinchangtingcheOptions: [],
				jinchangtingcheIndex: 0,
																																																																												// 登陆用户信息
				user: {},
                                ro:{
		...
		async onLoad(options) {
        		this.ruleForm.jinchangshijian = this.$utils.getCurDateTime();
			...
			this.user = res.data;
						...
						// ss读取
												...
												this.ruleForm.chepaihao = this.user.chepaihao
																														...
																														// 自定义下拉框值
			this.jinchangtingcheOptions = "是,否".split(',')
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`jinchangtingche`, this.ruleForm.id);
			...
			if(options.cross){
				...
				for (var o in obj){
					...
					if(o=='jinchangtingche'){
					this.ruleForm.jinchangtingche = obj[o];
					this.ro.jinchangtingche = true;
					continue;
					}
					if(o=='xiaoshidanjia'){
		...
		methods: {
			...
			// 多级联动参数
																																							...
																																							// 日长控件选择日期时间
			jinchangshijianConfirm(val) {
				console.log(val)
				this.ruleForm.jinchangshijian = val.result;
				this.$forceUpdate();
			},
																																										
																											// 下拉变化
			jinchangtingcheChange(e) {
				this.jinchangtingcheIndex = e.target.value
				this.ruleForm.jinchangtingche = this.jinchangtingcheOptions[this.jinchangtingcheIndex]
			},
			...
			async onSubmitTap() {
					...
					return
																																																																																																																																																																																																																																																																																																...
																																																																																																																																																																																																																																																																																																if(this.ruleForm.id){
					await this.$api.update(`jinchangtingche`, this.ruleForm);
				}else{
					await this.$api.add(`jinchangtingche`, this.ruleForm);
				}
