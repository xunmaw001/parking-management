<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="是否预约" prop="shifouyuyue">
          <el-select v-model="ruleForm.shifouyuyue" placeholder="请选择是否预约">
            <el-option
                v-for="(item,index) in shifouyuyueOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="是否预约" prop="shifouyuyue">
	      <el-input v-model="ruleForm.shifouyuyue"
                placeholder="是否预约" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.tupian = true;
          ...
          if(o=='shifouyuyue'){
            this.ruleForm.shifouyuyue = obj[o];
	    this.ro.shifouyuyue = true;
            continue;
          }
          if(o=='xiaoshidanjia'){
      ...
      this.$http({
		...
		if(json.chepaihao!=''&&json.chepaihao){
          ...
          this.$message.error(data.msg);
            ...
            this.shifouyuyueOptions = "是,否".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `yuyuetingche/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yuyuetingche/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yuyuetingcheCrossAddOrUpdateFlag = false;
                  this.parent.search();
